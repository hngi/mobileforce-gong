...
import 'package:team_mobileforce_gong/UI/password_reset_success.dart';
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    Container(
                      ...
                      child: Center(child: Image.asset('assets/images/gong_text_logo.png')),
                    ),
                    Padding(
                      padding: EdgeInsets.only(left: 0.0, top: sizeConfig.yMargin(context, 3.0)),
                      child: Text('Reset Password',
                        style: TextStyle(fontSize: sizeConfig.textSize(context, 7), fontFamily: 'Gilroy'),
                      ),
                    ),
                    Container(
                      width: sizeConfig.yMargin(context, 35.0),
                      padding: EdgeInsets.only(left: 0.0, top: sizeConfig.yMargin(context, 2.0)),
                      child: Text('Enter your email address below and we’ll send you password reset instructions',
...
//                        border: InputBorder.none,
                    ...
                    Builder(
                      builder: (context) => Container(
                        margin: EdgeInsets.only(top: sizeConfig.yMargin(context, 7.5)),
                        child: Center(
                          child: RaisedButton(
                            onPressed: () {
                            ...
                            padding: EdgeInsets.symmetric(vertical: sizeConfig.yMargin(context, 2.0), horizontal: sizeConfig.xMargin(context, 15.0)),
                            color: Colors.blue,
                            textColor: Colors.white,
                            child: Text('Log in'.toUpperCase(), style: TextStyle(fontFamily: 'Gilroy', fontWeight: FontWeight.w800),),
                          ),
                        ),
                      ),
                    ),
...
//    'Email sent to ' + emailText : 'Invalid email,  make sure your email is correct.';
    if(emailText.isNotEmpty) {
        return regex.hasMatch(emailText)? EmailValidate.SUCCESS : EmailValidate.INVALID;
    } else return EmailValidate.NONE;
  }

  void _navigateToSuccess(BuildContext context) {
    Navigator.push(context, MaterialPageRoute(builder: (context) => PasswordResetSuccess()));
  }

  void _showToast(BuildContext context) {
    final messages = <String>['Email address is required.',
...
//                          margin: const EdgeInsets.only(left: 0.0, top: 200),
            child: Text('Don\'t have an account?', style: TextStyle(fontFamily: 'Gilroy'),),
          ),
          InkWell(
            onTap: () {
